tasksController = function() {
	...
	function renderTable() {
	    $.each($(taskPage).find('#tblTasks tbody tr'), function(idx, row) {
	    	var data = $(row).find('[datetime]').text();
	    	data = data.substring(0, 10);
	    	//var arrData = data.split("-");
	    	//alert(Date.parseExact(data, "yyyy-MM-dd"));
	    	//alert(Date.parse(data));
	      var due = Date.parseExact(data, "yyyy-MM-dd");
	      //alert(due)
	     //alert (due.compareTo(Date.today()))
	...
	return {
				...
				if (initialised) {
					...
					//Verificação de tarefas realizadas aplicando as cores que está no css
				       $(taskPage).find('#tblTasks tbody').on('click', '.completeTask', function(evt) {           
					  storageEngine.findById('task', $(evt.target).data().taskId, function(task) {
					    task.completed = true;
					    storageEngine.save('task', task, function() {
					    	tasksController.loadTasks();
